...
window.actualizarReloj = function(id, valor, titulo) {
    if (typeof RGraph === 'undefined') return;
...
function activarMonitorDeProgreso() {
    ...
    const intervalMonitor = setInterval(async () => {
        try {
            // Usamos la función global para pedir el progreso
            const data = await callGoogleScript('get_progress');
            const progreso = data.reply; // Ajustado a tu estructura de retorno
...
function finalizarVisualmente(progreso) {
    ...
    if (modal) {
        const total = progreso.procesados + progreso.faltantes;
